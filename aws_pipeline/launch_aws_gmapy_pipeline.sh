...
branch_name="$1"
wait_arg="$2"
instance_info_file="../evaluation/aws_instance_info_${branch_name}.json"
...
if [ -e "$instance_info_file" ]; then
    ...
    if [ "$wait_arg" == "wait" ]; then
        bash get_results.sh "$branch_name"
    fi
    exit 0
fi

echo "prepare and run tensorflow for branch '$branch_name' on AWS machine..."
...
ssh -o StrictHostKeyChecking=no -i ephemeral-key.pem -t $login \
    "bash $rem_home/$pipe_script remote '$branch_name'"
