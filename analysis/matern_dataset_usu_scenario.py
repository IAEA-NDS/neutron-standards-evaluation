...
# expmask = exptable.REAC.str.match('MT:3-R1:9-R2:8')
expmask = ~expmask
...
def get_usu_cov(exptable, s, r):
    ...
    for reac in usu_reacs: 
        ...
        curdatasets = curdt.NODE.drop_duplicates().to_numpy()
        for curds in curdatasets:
            ccurdt = curdt[curdt.NODE == curds]
            idx = ccurdt.index.to_numpy() 
            ens = ccurdt.ENERGY.to_numpy()
            curcov = get_matern32_cov(ens, ens, s, r)
            usu_cov[np.ix_(idx, idx)] = curcov
    return usu_cov


usu_cov = get_usu_cov(exptable, 0.03, 1.0)
new_expcov = csr_matrix(expcov_rel + usu_cov)

postcov, postcov_abs = calc_postcov2(
    rS, new_expcov, exptable.PRED, rpriortable.PRED, block_starts, block_stops, ret_abscov=True
)
...
rrS = rS[sacs_idx,:]
np.sqrt(rrS @ postcov_abs @ rrS.T) / exptable.loc[sacs_idx, 'DATA'].to_numpy()
