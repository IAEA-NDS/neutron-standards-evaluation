...
smat_np = np.stack(smat_cols, axis=1)
smat_ds = tf.constant(smat_np, dtype=tf.float64)
...
def create_like_cov_fun(smat, colmap, expcov_linop):
    def diag_fun(u):
        # softplus magnitude parameterization: unconstrained covpars,
        # asymptotically linear for active bands, exponentially
        # decaying (no quartic saddle at zero) for inactive ones
...
save_objects(f'{outdir}/01_model_preparation_output.pkl', locals(),
             ...
             'num_covpars', 'priortable', 'exptable', 'expcov', 'like_cov_fun', 'compmap', 'restrimap', 'smat_np', 'col2ds')
