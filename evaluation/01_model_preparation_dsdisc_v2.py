...
num_covpars = n_shared + len(ds_records)
...
red_usu_df = pd.DataFrame(sh_records + ds_records)
usu_df = red_usu_df
print(f'combined USU model: {n_shared} shared knots + '
...
def create_like_cov_fun(smat, colmap, expcov_linop):
    def diag_fun(u):
        # softplus magnitude parameterization for all bands
        sig = tf.math.softplus(tf.gather(u, colmap))
        return tf.square(sig) + 1e-7
    return LowRankCovarianceModel(
        expcov_linop, tf.constant(smat, dtype=tf.float64), diag_fun
    )


like_cov_fun = create_like_cov_fun(smat_np, col2par, expcov_linop)
...
# weak half-normal restraint on the per-dataset band magnitudes
# (rejection of a dataset should saturate, not run away); the shared
# floor bands are left unrestrained
band_idcs = len(adj_idcs) + n_shared + np.arange(num_covpars - n_shared)
bandprior = DistributionForParameterSubset(
    SoftplusHalfNormal(0.2), len(adj_idcs) + num_covpars, band_idcs
)
post = UnnormalizedDistributionProduct(
    [prior, likelihood, posbarrier, bandprior]
)
